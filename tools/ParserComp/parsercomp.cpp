...
map<string, bool> settings_map;
// unmangle_mode
// subobject_mode
// tag_mode
// arrayrecognize_mode

void generateCreateObjectStub(ostream& fout)
{
	bool arrayrecognize_mode = settings_map["arrayrecognize_mode"];
	fout<<"\t\t\t\t"<<"//[create object]."<<endl;
	if(arrayrecognize_mode)
	{
		fout<<"\t\t\t\t"<<"QScriptValue newItemValue;"<<endl;
		fout<<"\t\t\t\t"<<"if(curElem.attribute(\"_\")==\"_\")"<<endl;
		fout<<"\t\t\t\t\t"<<"newItemValue = model->getEngine()->newArray(0);"<<endl;
		fout<<"\t\t\t\t"<<"else"<<endl;
		fout<<"\t\t\t\t\t"<<"newItemValue = model->getEngine()->newObject();"<<endl;
	}
	else
	{
...
void generateWriteTagStub(ostream& fout)
{
	bool tag_mode = settings_map["tag_mode"];
	if(tag_mode)
...
{
	bool subobject_mode = settings_map["subobject_mode"];
	bool unmangle_mode = settings_map["unmangle_mode"];
	...
	{
		fout<<"\t\t\t\t"<<"//[write this Item To Parent as property]"<<endl;
		fout<<"\t\t\t\t"<<"writePropertyToModel(newItemValue, taskList[curTask].taskParent, "
			<< (!unmangle_mode ? "curElem.tagName()" : "unmangleElementName(curElem.tagName())") <<");"<<endl;	
	}
	fout<<endl;	
}
...
void generateWriteAttributesStub(ostream& fout, int i/*hint*/, int indentLength)
{
	...
	if(i>=0)
	{
		...
		for(int k=0;k<state_struct::registry[i]->transform_rules.size();k++)
		{
			...
			{
				fout<<indent<<"\t"<< (childAttribRulesWritten ? "else " : "")
					<<( !unmangle_mode ? "if(curElem.attributes().item(attrIndex).nodeName()==\"" : "if(unmangleElementName(curElem.attributes().item(attrIndex).nodeName())==\"")
					<<state_struct::registry[i]->transform_rules[k].attributeName<<"\""<<generateAssertionClause(state_struct::registry[i]->transform_rules[k].assertionString)<<")"<<endl;
	...
	fout<<indent<<"\t"<<"{"<<endl;
	fout<<indent<<"\t\t"<<"newItemValue.setProperty(  " << 
		(!unmangle_mode ? "curElem.attributes().item(attrIndex).nodeName()" : "unmangleElementName(curElem.attributes().item(attrIndex).nodeName())")<<", curElem.attributes().item(attrIndex).nodeValue());"<<endl;
	fout<<indent<<"\t"<<"}"<<endl;
...
int parse(const char* _inputfile, const char* _outputfile)
{
	...
	settings_map["unmangle_mode"] = false;
	settings_map["subobject_mode"] = true;
	settings_map["tag_mode"] = true;
	settings_map["arrayrecognize_mode"] = false;
			...
			fout<< "\t\t\t" << "{" <<endl;
			{
				...
				for(int j=0;j<commands.size();j++)
				{
					if(commands[j]=="standardObject"||commands[j]=="createObject")
					{
						...
						generateCreateObjectStub(fout);
						generateWriteTagStub(fout);
					...
					else if(commands[j]=="setUnmangleModeOn")
					{
						settings_map["unmangle_mode"] = true;
					}
					else if(commands[j]=="setUnmangleModeOff")
					{
						settings_map["unmangle_mode"] = false;
					}
					else if(commands[j]=="setSubObjectModeOn")
					{
						settings_map["subobject_mode"] = true;
					}
					else if(commands[j]=="setSubObjectModeOff")
					{
						settings_map["subobject_mode"] = false;
					}
					else if(commands[j]=="setTagModeOn")
					{
						settings_map["tag_mode"] = true;
					}
					else if(commands[j]=="setTagModeOff")
					{
						settings_map["tag_mode"] = false;
					}
					else if(commands[j]=="setArrayRecognizeModeOn")
					{
						settings_map["arrayrecognize_mode"] = true;
					}
					else if(commands[j]=="setArrayRecognizeModeOff")
					{
						settings_map["arrayrecognize_mode"] = false;
					}
